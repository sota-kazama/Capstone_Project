...
<input type="checkbox" name="correct_answers[]"
       value="<?= $i ?>" <?= in_array($i, $corrects) ? 'checked' : '' ?>> 正解
</div>
</div>
<?php endfor; ?>
</div>
...
<div class="mb-3">
    ...
    <div class="mt-2">
        ...
        <img src="../uploads/<?= htmlspecialchars($editQuestion->image_path) ?>" 
             alt="問題画像" style="max-width:200px; max-height:150px;">
        <p class="text-muted">変更する場合は新しい画像を選択してください。</p>
        <!-- 画像削除用チェックボックス（任意） -->
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="delete_image" value="1">
            <label class="form-check-label">この画像を削除する</label>
        </div>
    </div>
    <?php endif; ?>
</div>
...
<button name="search" class="btn btn-secondary">検索</button>
</form>
</div>
...
<button class="btn btn-sm btn-outline-danger"
        onclick="confirmDelete(<?= $q->q_number ?>)">削除</button>
</div>
</div>
<?php endforeach; ?>
</div>
...
<div id="questionList" class="collapse <?= $isSearching ? '' : 'show' ?>">
<div class="list-group mt-3">
