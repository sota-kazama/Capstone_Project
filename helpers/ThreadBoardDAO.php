...
class ThreadBoardDAO
{
    ...
    // 次の投稿番号取得
    private function getNextToukouNumber(int $thread_number)
    {
        $dbh = DAO::get_db_connect();

        $sql = "SELECT ISNULL(MAX(toukou_number), 0) + 1
                FROM thread_data
                WHERE thread_number = ?";
    ...
    public function insertPost(int $thread_number, int $user_id, string $content)
{
    ...
    $sql = "INSERT INTO thread_data
            (thread_number, user_id, post_content, created_ad, update_at)
