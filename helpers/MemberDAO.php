...
class Member
{
    ...
    public ?int $u_correct_count;  // 正答数カウント
...
class MemberDAO
{
    ...
    public function get_member(string $mail_address, string $pass_word)
    {
        ...
        $member = $stmt->fetchObject('Member'); // Memberクラスのインスタンス取得

        if ($member !== false && password_verify($pass_word, $member->pass_word)) {
            $this->update_access_date($member->user_id); // 最終アクセス日更新
            return $member;
        }
        return false;
    }
    ...
    {
        $dbh = DAO::get_db_connect();
        $sql = "UPDATE master_user SET access_date = GETDATE() WHERE user_id = :user_id";
        $stmt = $dbh->prepare($sql);
    ...
    {
        $dbh = DAO::get_db_connect();
        $sql = "SELECT user_id, user_name, mail_address, u_admin, member_type, access_date
                FROM master_user ORDER BY user_id ASC";
        $stmt = $dbh->prepare($sql);
        $stmt->execute();
    ...
    public function getAllMembersPaged(int $page = 1, int $perPage = 50): array
    {
        ...
        $totalCount = (int)$dbh->query($countSql)->fetchColumn();
    ...
    public function updateMemberAccount(int $user_id, string $user_name, string $mail_address, ?string $password_hashed, int $u_admin): bool
    {
        ...
        $sql = "UPDATE master_user
                SET user_name = :user_name,
                    ...
                    u_admin = :u_admin,
                    update_at = GETDATE()";
        if ($password_hashed !== null) $sql .= ", pass_word = :pass_word";
        ...
        $stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
        if ($password_hashed !== null) $stmt->bindValue(':pass_word', $password_hashed, PDO::PARAM_STR);
